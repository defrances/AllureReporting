  ...
  return new Promise(async (resolve, reject) => {
    await Promise.all(
      LANGUAGES.map((lang) => lang.id).map(async (lang) => {
        ...
        // locale.default might be already an object (from webpack) or a JSON string
        const translationData =
          typeof locale.default === "string" ? JSON.parse(locale.default) : locale.default;

        addTranslation(lang, translationData);
      }),
    );
