...
export const generateStaticFiles = async (payload: {
  ...
  if (!singleFile) {
    try {
      const webAllure2PackagePath = require.resolve("@allurereport/web-allure2/package.json");
      const webAllure2Root = join(webAllure2PackagePath, "..");
      const translationsPath = join(webAllure2Root, "src", "translations");
      const translationFiles = await readdir(translationsPath);

      for (const translationFile of translationFiles) {
        if (translationFile.endsWith(".json")) {
          const translationFilePath = join(translationsPath, translationFile);
          const translationContent = await readFile(translationFilePath);
          await reportFiles.addFile(joinPosix("translations", translationFile), translationContent);
        }
      }
    } catch (err) {
      // If translations directory is not found, continue without translations
      // This might happen in some build configurations
    }
  }
